...
const Signup = () => {
      ...
      const timer = setTimeout(() => {
        try {
          ...
          if (emailRegex.test(form.email)) {
            setEmailCheck({ status: "success", message: "사용 가능한 이메일입니다." });
          } else {
            setEmailCheck({ status: "error", message: "올바른 이메일 형식이 아닙니다." });
          }
        } catch (err) {
          setEmailCheck({ status: "error", message: "이메일 확인 중 오류가 발생했습니다." });
        }
      }, 1000); // 1초 후 자동 확인
      ...
      const timer = setTimeout(() => {
        try {
          ...
          if (form.user_id.length >= 4) {
            ...
            setIdCheck({ status: "error", message: "아이디는 4자 이상이어야 합니다." });
          }
        } catch (err) {
          setIdCheck({ status: "error", message: "아이디 확인 중 오류가 발생했습니다." });
        }
      }, 1000); // 1초 후 자동 확인
      ...
      const timer = setTimeout(() => {
        try {
          if (form.password === form.passwordConfirm) {
            setPasswordCheck({ status: "success", message: "비밀번호가 일치합니다." });
          } else {
            setPasswordCheck({ status: "error", message: "비밀번호가 일치하지 않습니다." });
          }
        } catch (err) {
          setPasswordCheck({ status: "error", message: "비밀번호 확인 중 오류가 발생했습니다." });
        }
      }, 1000); // 1초 후 확인
  ...
  return (
    <div className="mobile-wrapper">
      ...
      {message && (
        <div className="signup-message" style={{ color: message.includes("성공") ? "green" : "red" }}>
          {message}
        </div>
      )}
