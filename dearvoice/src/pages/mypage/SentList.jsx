...
const SentList = () => {
  ...
  const filteredLetters = safeLetters.filter((item) => {
    ...
    if (item.type === "sky") {
      ...
      recipientValue =
        item.recipients?.map((r) => r.display_id || r.email).join(", ") || "";
    }
  ...
  const handleDelete = async (letterId, letterType) => {
    ...
    try {
      await axiosInstance.delete(
        `/api/mypage/sent/delete/${letterType}/${letterId}/`
      );
    ...
    <div className="sentlist-wrapper">
      ...
      <div className="sentlist-searchbar-wrapper">
        <div className="sentlist-searchbar-inner">
          ...
          />
          <FaSearch className="sentlist-search-icon" onClick={handleSearch} />
        </div>
      </div>
          ...
          currentLetters.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`sent-item ${
                  colorClass[item.paper_color] || "sent-item-gray"
                }`}
              >
                <span className="sent-title">
                  [ {item.title?.slice(0, 15)} ]
                </span>
                <span className="sent-user">{recipientValue}</span>
